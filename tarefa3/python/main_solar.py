...
def initialize():
  ...
  earth_node = Node(trf=earth_spin, apps=[earth_mat], shps=[earth_disk])
  ...
  root = Node(shader, nodes=[sun_node, earth_branch])
...
def main():
  ...
  win = glfw.create_window(800, 800, "sun-earth-moon", None, None)
  if not win:
    glfw.terminate()
    return
